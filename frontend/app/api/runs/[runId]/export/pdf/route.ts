...
export async function GET(
  ...
  try {
    const res = await fetch(backendUrl(`/api/runs/${runId}/export/pdf`));
    if (!res.ok) {
    ...
    return new Response(res.body, {
      ...
      headers: {
        ...
        "Content-Disposition": res.headers.get("Content-Disposition") || `attachment; filename="spot-on-${runId}.pdf"`,
      },
    });
  } catch {
